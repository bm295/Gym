...
- [x] Establish accessible focus states, keyboard operation, semantic headings,
  and sufficient color contrast for all primitives.
- [x] Replace the default Bootstrap starter appearance with the operational SaaS
  visual system while retaining only useful Bootstrap layout utilities.

## 2. App Shell and Localization

- [x] Replace the starter layout with a responsive application shell.
- [x] Add a grouped sidebar for Overview, Members, Plans & Subscriptions, Front
  Desk, Finance, and Administration.
- [x] Add a top bar with branch context, global quick search entry point,
  notifications, language switcher, and staff profile menu.
- [x] Implement compact/collapsible navigation for tablet widths and usable
  responsive behavior for mobile widths.
- [x] Introduce a UI-only current staff member and selected branch context.
- [x] Add a client-side localization dictionary/resource layer for Vietnamese and
  English navigation, labels, messages, statuses, and help text.
- [x] Persist the selected language and selected branch for the browser session.

## 3. Interactive Domain Prototypes

- [x] Create realistic UI-only seed data for tenants, branches, staff, members,
  membership plans, subscriptions, payments, check-ins, audit entries, and
  reports.
- [x] Create a UI state service that owns prototype records and applies create,
  edit, lock, archive, sell, renew, payment, void, and check-in actions in the
  active browser session.
- [x] Clearly identify simulated actions in modules that do not yet have backend
  support; do not imply that these actions are persisted remotely.
- [x] Keep the existing API client and backend contracts unchanged and separate
  from the prototype state.

## 4. Operational Screens

- [x] Build the Dashboard with active-member, today’s check-in, today’s revenue,
  and expiring-subscription cards; include branch filtering and action shortcuts.
- [x] Build Member List with search, status/branch filters, sortable columns,
  pagination, and create-member entry point.
- [x] Build Member Detail with profile summary, current subscription status,
  payments, check-in history, and a chronological activity timeline.
- [x] Build create/edit member forms with inline validation and an explicit
  cancel/unsaved-changes experience.
- [x] Build Plan List and Plan Detail with plan status, duration, price, visit
  limit, cross-branch policy, and archive interaction.
- [x] Build subscription sale, renewal, and subscription-list flows using plan
  snapshots, date range, home branch, amount paid, and activation status.
- [x] Build Payment History with date/member/status filters, payment details, and
  a reason-required void confirmation prototype.
- [x] Build Attendance History with date, branch, and member filters.
- [x] Build Branch, Staff, Reports, and Audit Log prototype screens with domain
  appropriate tables, filters, empty states, and role/status badges.

## 5. Reception Check-in

- [x] Replace UUID entry with the selected branch from the app context.
- [x] Make member-code/phone lookup the primary visual and keyboard-focused
  action.
- [x] Display a member summary card with name, member code, photo placeholder,
  member status, plan name, validity period, permitted branch status, and
  remaining visits or unlimited entitlement.
- [x] Support clear prototype outcomes: successful unlimited plan, successful
...
- [x] Present a high-confidence success state with timestamp and an immediate
  "check in another member" action.
- [x] Present actionable rejection states that explain the business reason and
  direct the receptionist to the relevant member/subscription workflow.
- [x] Add a recent check-in list for the selected branch to help the receptionist
  verify the latest attendance activity.

## 6. Quality and Verification

- [x] Add component tests for app-shell navigation, language switching, branch
  selection, responsive sidebar state, and persisted session state.
- [x] Add UI tests for tables, filters, pagination, empty states, form
  validation, confirmation modals, and prototype data mutations.
- [x] Add check-in UI tests for all supported success and rejection outcomes.
- [x] Smoke-test the key desktop and tablet paths: dashboard, member lookup,
  subscription sale/renewal prototype, payment void prototype, and check-in.
- [x] Verify keyboard navigation, focus order, screen-reader status messages, and
  color contrast on key operational flows.
- [x] Confirm no backend code, API contracts, database schema, or authentication
